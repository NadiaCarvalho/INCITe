...
def start_draw(_oracle, _offsets=None, size=(900*4, 400*4)):
    """
...
def draw_offsets(oracle, offsets, current_state, image, width=WIDTH, height=HEIGHT):
    """
    ...
    sum_offset = (offsets[-1] + 2) - offsets[0]
    make_offsets = [-0.2] + list(map((lambda x: x + 0.8), offsets))

    print(n_states)

    for i in range(n_states):
        # draw circle for each state
        x_pos = ((make_offsets[i] / sum_offset) * width) + 0.5 * (1.0 / sum_offset) * width
...
def draw_oracles(oracles, offsets, current_state, image, width=WIDTH, height=HEIGHT):
    ...
    for key, oracle in oracles.items():
        ...
        if key in offsets:
            al = draw_offsets(oracle, offsets[key], current_state, new_image, width, height=int(height/len(oracles)))
        else:
            al = draw(oracle, current_state, new_image, width, height=int(height/len(oracles)))
        images.append(al)
