...
def parseLine(to_parse):
    # Get Notes and Rests only
    stream_notes_rests = to_parse.flat.notesAndRests.stream()

    # Get offsets of beginnings of measures
    measure_offsets = [measure.offset for measure in to_parse.recurse(classFilter='Measure')]
    ...
    for dynamic in to_parse.flat.getElementsByClass(dynamics.Dynamic):
    ...
    keys = [keySignature for keySignature in to_parse.flat.getElementsByClass(key.KeySignature)]
    ...
    time_sigs = [timeSignature for timeSignature in to_parse.flat.getElementsByClass(meter.TimeSignature)]
    ...
    double_barlines = [barline.offset for barline in to_parse.flat.getElementsByClass(bar.Barline) if barline.type == 'double']
    for d_bar in double_barlines:
        utils.getEventsAtOffset(events, d_bar.offset)[0].addViewpoint(Viewpoint('double_bar_before', True))

    repeats =  [repeat for repeat in to_parse.flat.getElementsByClass(bar.Repeat)]
